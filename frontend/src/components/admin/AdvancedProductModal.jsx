...
import RichTextEditor from "./RichTextEditor"; // Assuming path is correct
import toast from "react-hot-toast";
import LoadingSpinner from "../UI/LoadingSpinner"; // Assuming path
...
// Add CSS for .form-label, .form-error, .input in your global CSS
...
const AdvancedProductModal = ({
  ...
  const {
    ...
    defaultValues: useMemo(() => {
      ...
      return {
        ...
        category:
          productToEdit?.category?.slug || MOCK_CATEGORIES[0]?.slug || "",
        ...
        images:
          productToEdit?.images?.map((img) => ({
            ...
            isNew: false,
            file: null,
            publicId: img.publicId || generateUniqueId("existing"),
          })) || [],
        status: productToEdit?.status || "published",
        showQuantityLeft: isEditingMode
          ? productToEdit?.showQuantityLeft !== undefined
            ? productToEdit.showQuantityLeft
            : true
          : false,
  ...
  const [price, discount, categorySlug, productName] = watch([
    "price",
    "discount",
    "category",
    "name",
  ...
  const getSubcategories = useMemo(
    () =>
      MOCK_CATEGORIES.find((c) => c.slug === categorySlug)?.subcategories || [],
    [categorySlug]
  );
  ...
  }, [productToEdit, isEditing, isOpen, reset]);
  ...
  const handleImageChange = (e) => {
    ...
    console.log(
      "[handleImageChange] Current images count:",
      currentImagesValue.length
    );
    console.log("[handleImageChange] Files to process:", filesToProcess.length);
    ...
    // Process files synchronously into a new array
    const newImagesToAdd = filesToProcess
      .map((file) => {
        ...
        console.log(
          `[handleImageChange] Generated ID for ${file.name}: ${uniqueId}`
        );
        return {
          url: URL.createObjectURL(file), // Use object URL
          file: file,
          isNew: true,
          publicId: uniqueId,
        };
      })
      .filter(Boolean); // Remove nulls from oversized files

    console.log(
      "[handleImageChange] New image objects created:",
      newImagesToAdd.length
    );

    // Combine current images and new images *before* setting state
    if (newImagesToAdd.length > 0) {
      const updatedImages = [...currentImagesValue, ...newImagesToAdd];
      console.log(
        "[handleImageChange] Updating state with combined array:",
  ...
  // --- FINAL ATTEMPT: Image Deletion ---
  const removeImage = (publicIdToRemove) => {
    console.log("[removeImage] Request to remove ID:", publicIdToRemove);
    const currentImagesValue = getValues("images") || [];
    console.log(
      "[removeImage] Current IDs:",
      currentImagesValue.map((img) => img.publicId)
    );
    ...
    if (!imageToRemove) {
      console.warn("[removeImage] Image not found with ID:", publicIdToRemove);
    ...
    console.log(
      "[removeImage] IDs after filtering:",
      updatedImages.map((img) => img.publicId)
    );

    // Update the form state
    setValue("images", updatedImages, { shouldDirty: true });
    console.log("[removeImage] State update successful.");

    // Clean up blob URL
    if (imageToRemove.url && imageToRemove.url.startsWith("blob:")) {
      URL.revokeObjectURL(imageToRemove.url);
      console.log("[removeImage] Revoked Object URL:", imageToRemove.url);
    }
  };

  const handleModalSubmit = (data) => {
    /* ... submit logic ... */ const slug =
  ...
  const onError = (formErrors) => {
    /* ... error handling ... */ console.error("Form Errors:", formErrors);
          ...
          <div className="p-6 bg-surface dark:bg-dark-surface rounded-lg border border-border dark:border-dark-border/50 shadow-sm space-y-4">
            ...
            <AnimatePresence>
              {currentImages && currentImages.length > 0 && (
                <motion.div
                  layout // Enable layout animation on the container
                  className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-3"
                >
                  {currentImages.map((img) => (
                    <motion.div
                      key={img.publicId} // CRITICAL: Use the unique publicId
                      layout // Animate layout changes for each item
                    ...
                    >
                      <img
                        ...
                        // Clean up blob URL when component unmounts or img.url changes
                        onLoad={(e) => {
                          if (
                      ...
                      <button
                        type="button"
                        // Ensure this calls the correct function with the unique ID
                        onClick={(e) => {
                          e.stopPropagation(); // Prevent potential parent clicks
                          removeImage(img.publicId);
                        }}
                        className="absolute top-1 right-1 p-1 bg-red-600/90 text-white rounded-full shadow-md opacity-0 group-hover:opacity-100 transition-opacity duration-200 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-1 focus:ring-offset-card hover:bg-red-700 z-10" // Added z-index
            ...
            <Controller
              ...
              render={(
                { fieldState: { error } } // Get error state directly
              ) => (
                <>
                  <label
                  ...
                  >
                    ...
                    <input
                      ...
                      multiple
                      onChange={handleImageChange} // Use the robust handler
                      className="hidden"
                      disabled={isSaving || (currentImages?.length || 0) >= 5} // Use watched state for disabled check
                    />
                  </label>
  ...
  <label
    className={`flex items-center justify-center min-w-[40px] h-9 px-3 border rounded-lg cursor-pointer transition-all font-medium text-sm shadow-sm ${
      selected
        ...
        : "border-border dark:border-dark-border text-muted dark:text-dark-muted bg-surface dark:bg-dark-surface hover:border-gray-400 dark:hover:border-gray-500 hover:text-primary dark:hover:text-dark-primary"
    }`}
  >
    {" "}
    <input
